...
- Mr.SIP defines a custom `FOUND` log level (between `INFO` and `WARNING`) specifically for "live host found" / "valid extension found" events — that's what produces the `[ FOUND ]` lines you see in SIP-NES/SIP-ENUM output.
...
- **SIP-ENUM, target is a modern PJSIP-based PBX**: `chan_sip` and `chan_pjsip` are Asterisk's two SIP channel drivers — `chan_sip` is the original, in-house implementation; `chan_pjsip` is the newer one built on the external PJSIP library, and has been the only option since Asterisk 21 (`chan_sip` was deprecated in 17, removed in 21 - see [lab-guide.md](lab-guide.md#vm-lab--classic-chan_sip--trixbox-ce-or-similar-eol-asterisk-distros) for what that means for picking a lab target). SIP-ENUM's 401/403-means-"exists" heuristic was designed against classic `chan_sip`, which really does return a distinct code for a nonexistent extension. Some PJSIP configurations instead reject *every* unmatched request with the same 401/403, which would otherwise look identical to "extension exists" under that heuristic. SIP-ENUM runs one pre-flight probe per target (threaded, same as the real scan) with a random, guaranteed-nonexistent username before the real scan; if that gets a 401/403 back, it warns you up front that results may be unreliable against that target. Scanning several targets at once, the warning is aggregated rather than repeated per host: one line naming each target up to 5, a single count-only summary beyond that. See [lab-guide.md](lab-guide.md) for a way to compare this directly against a classic `chan_sip` target where the heuristic behaves as originally designed.
